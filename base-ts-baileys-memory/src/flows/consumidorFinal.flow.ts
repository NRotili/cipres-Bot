...
import { consumidorFinalConsultaEnviosFlow, consumidorFinalConsultaHorariosFlow, consumidorFinalConsultaPreciosFlow } from "./consumidorFinalConsulta.flow";

const consumidorFinalConsultaFlow = addKeyword(EVENTS.ACTION)
    .addAnswer("Qué consulta te interesa? 🤔", {delay: 1000})
    .addAnswer(['1️⃣. Precios',
        '2️⃣. Horarios',
        '3️⃣. Envíos',
        '9️⃣. Volver'], {delay: 1000, capture: true},
    async (ctx, ctxFn) => {
        const bodyText: string = ctx.body.toLowerCase();
        const keywords: string[] = ['1', 'precios', '2', 'horarios', '3', 'envíos', '9', 'volver'];
